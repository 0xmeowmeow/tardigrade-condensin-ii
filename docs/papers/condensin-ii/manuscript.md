...
date: "Preprint, version 4, 20 September 2026 · doi:10.5281/zenodo.22846138 (all versions: 10.5281/zenodo.22832510)"
abstract: |
...
## The tardigrade CAP-H2 proteins are predicted to bind both SMC heads
...
The C-terminal regions of five of the six tardigrade CAP-H2 proteins are predicted to bind SMC4 with interface confidence close to human CAP-H2 (Fig. 4a): ipTM 0.78 and 0.79 for the two *H. exemplaris* proteins, 0.76 for *R. varieornatus* b, and 0.81 for both *Paramacrobiotus* proteins, against 0.83 for human. The exception is *R. varieornatus* a (0.68, unstable across samples). The local picture is less uniform than the ipTM: the mean predicted aligned error of the confident interface residues is 2.6–3.8 Å for human, both *H. exemplaris* proteins, *R. varieornatus* b and *Paramacrobiotus* b, but 9.3 Å for *Paramacrobiotus* a. The condensin I kleisin CAP-H of *H. exemplaris*, which should bind the same head, scores 0.81. Three controls score lower: the same regions offered SMC2 instead (0.31 and 0.36; human 0.59), and residue-shuffled regions (0.19–0.45).

With the first SMC2 construct, the N-terminal regions gave no clear signal (Fig. 4b). Human CAP-H2 was placed on the SMC2 neck at 0.67, but the tardigrade N-terminal regions scored 0.29–0.48 with no confidently placed interface residues; *H. exemplaris* b (0.44) scored above its wrong-partner and scrambled controls (0.29 and 0.26), the others did not. We repeated the prediction with a longer SMC2 construct that includes more of the neck, with five samples per pair (Methods; Fig. 4c). The N-terminal regions of the b copies of *H. exemplaris* and *R. varieornatus* are now placed on SMC2 in all five samples: ipTM 0.62–0.67 (median 0.65) and 0.64–0.69 (median 0.65), against 0.57–0.61 (median 0.58) for human CAP-H2. In the selected models their interfaces have 49 and 39 confidently placed kleisin residues in contact with SMC2 (pLDDT at least 70), against 39 for human (the other samples: 40–44 and 37–38, human 38–42). Scrambled versions of the human and *H. exemplaris* b regions score medians of 0.19 and 0.11, with no confidently placed interface residues; the best of five scrambled human samples reaches 0.52. The b copies contact the same stretch of the SMC2 neck as human CAP-H2. Counting SMC2 residues touched by the confident kleisin residues, in each protein's own numbering, the two neck segments are 168–212 and 994–1040 (*H. exemplaris*) and 171–211 and 1000–1043 (*R. varieornatus*), against 169–210 and 986–1032 in human. *H. exemplaris* a (median 0.12) and *Paramacrobiotus* b (median 0.35) show no support with either construct; the a copies of the other two species were not retried. So in *H. exemplaris* and *R. varieornatus*, copy b is predicted to bind both SMC heads, in separately predicted interfaces; for these two, the failure with the first construct reflected the construct (the retry also used five samples instead of three). The fragment models do not show the whole ring closed at once. Figure 5 shows one of the confident C-terminal models in 3D.

![**Predicted binding of the CAP-H2 regions to the condensin ring.** Interface confidence (ipTM) of Boltz-2 predictions, best of three samples (five in c); bars show the range across samples. (a) CAP-H2 C-terminal regions with the same species' SMC4 head, the *H. exemplaris* condensin I kleisin CAP-H with SMC4 (green), and controls: the region offered SMC2 instead (wrong partner) or with its residues shuffled (grey). (b) CAP-H2 N-terminal regions with the first SMC2 head-and-neck construct, and the same controls. (c) N-terminal regions with a longer SMC2 construct (best of five samples): the b copies of *H. exemplaris* and *R. varieornatus* are placed as confidently as human CAP-H2; scrambled controls in grey. The dotted line marks 0.8, a convention from AlphaFold-Multimer for usually correct interfaces, not a calibration for these constructs.](figs/fig4-structure.pdf){width=100%}
...
**Limitations.** We used only annotated proteomes. Gene order places the duplication before the split between Hypsibioidea and Macrobiotoidea; a genome from another order (Apochela) or from a heterotardigrade would date it further back. The only heterotardigrade data we could examine, an *Echiniscus testudo* transcriptome (GDAL01), contains only a fragment of SMC4 among the condensin genes. The CNDH2_M domain was not detected. This may reflect divergence or a genuine loss of that region, which matters for binding to CAP-G2. Co-expression is correlative. We did not reanalyse the Hi-C data. The structure predictions are models, not measurements: they show that the tardigrade C-terminal regions, and the N-terminal regions of the b copies, are compatible with binding the SMC heads as a condensin II kleisin does, not that they do so in the cell. The N-terminal result depends on the length of the SMC2 construct, and the a copies of *R. varieornatus* and *Paramacrobiotus* were not retried with the longer one.
...
**Structure prediction.** We used Boltz-2 [@passaro2025] (version 2.2.1; three diffusion samples, three recycling steps, inference potentials on), with multiple sequence alignments from the ColabFold MMseqs2 server, on one 16 GB GPU. SMC head constructs join the N-terminal lobe, up to about 50 residues into the coiled coil, to the C-terminal lobe from about 50 residues before it, with a GGSGGSGGSGGS linker. Boundaries were taken from long helical runs in the human AlphaFold models (SMC2 1–195 + 1000–1197; SMC4 56–313 + 1105–1288) and mapped to each tardigrade protein through a MAFFT alignment. We report the ipTM of the best sample by Boltz confidence score, with its range over the three samples. Controls paired each region with the other SMC construct, or shuffled its residues. The N-terminal regions were then predicted again with a longer SMC2 construct (human 1–300 + 950–1197; *H. exemplaris* 1–298 + 957–1211, *R. varieornatus* 1–300 + 962–1217, *Paramacrobiotus* 1–300 + 942–1175, mapped as above) and five diffusion samples, for human CAP-H2, both *H. exemplaris* copies and the b copies of the other two species, with scrambled controls for human and *H. exemplaris* b. For every prediction we counted interface residues in the selected model: kleisin residues with pLDDT of at least 70 and any heavy atom within 5 Å of the SMC chain, with their mean predicted aligned error to the SMC residues within 8 Å (averaged per kleisin residue, then over residues). The SMC residues they touch are reported in each protein's own numbering, skipping the linker. The selected sample is the one with the highest Boltz confidence score (0.8 × complex pLDDT + 0.2 × ipTM), not necessarily the highest ipTM. Scrambled controls keep length, composition and partner, but also lose their sequence alignment (the server finds no homologues), so they control for evolutionary information as well as order; wrong-partner controls exist for human and *H. exemplaris* only.
...
- `condensin_check.py`, `caph2_profile.py`, `kleisin_tree.py`, `caph2_synteny.py`, `caph2_structure_inputs.py`, `caph2_structure_analysis.py`, `condensin_figdata.py`, `condensin_figures.py` and `hoencamp_fisher.py` (in `tools/genome/`);
- seed sets `condensin_caph2_seeds.fa` and `condensin_caph_seeds.fa`;
- the alignment and tree.

Code, derived data, alignments and trees: <https://github.com/0xmeowmeow/tardigrade-condensin-ii>. This preprint: Zenodo, all versions [doi:10.5281/zenodo.22832510](https://doi.org/10.5281/zenodo.22832510) (version 1: 10.5281/zenodo.22832511). Each claim is recorded with its source and page in the project's knowledge graph.

# Acknowledgements and use of AI

The analyses, figures and first draft of this manuscript were produced with an AI assistant (Claude, Anthropic) working under the author's direction. The author checked the results and takes responsibility for the content. For version 4, a second AI system (Codex, OpenAI) independently recomputed every structure-prediction number from the raw outputs and reviewed the analysis code; its review found the errors corrected below.

**Version history.** Version 2 added the structure predictions; version 3 the N-terminal regions with a longer SMC2 construct. Version 4 corrects version 3: the SMC2 residue numbers of the N-terminal contacts were off by the 12-residue linker (human neck segments 169–210 and 986–1032, not 168–232 and 992–1048; the tardigrade segments are corrected likewise); the statement that the b copies close the ring at both ends is limited to *H. exemplaris* and *R. varieornatus* and to separately predicted interfaces; *H. exemplaris* b did score above its controls with the first construct; the local interface error of *Paramacrobiotus* a is higher than for the other C-terminal predictions; and the controls' loss of sequence alignments is stated. The ipTM values, ranges, medians and interface counts were all reproduced.
